/* CTD | BANCO DE DADOS I | CHECKPOINT II*/
/* CAUÊ SILVEIRA | TURMA 1 | PART-TIME*/

/* SCRIPT DAS INSTRUÇÕES DML */
...
FROM OS
    ...
    INNER JOIN OFICINA AS O
        ON O.ID_OFICINA = OS.ID_OFICINA
WHERE
    OS.SITUACAO = 'ABERTA'
    AND
    O.NOME_OFICINA = 'HIDRAULICA'
ORDER BY
    OS.DATA_SOLICITACAO ASC;
...
FROM TECNICO AS TEC
    ...
    INNER JOIN OFICINA AS O
        ON O.ID_OFICINA = ESP.ID_OFICINA
GROUP BY
...
/* ATRIBUIR UM TÉCNICO PARA A EXECUÇÃO DA OS Nº 3 DA OFICINA DE ELÉTRICA */
UPDATE OS
SET
    ID_TECNICO = 3
WHERE
...
SET
    ...
    SITUACAO = 'CONCLUIDA',
    COMENTARIOS = 'FORAM SUBSTITUIDOS OS ANÉIS DE VEDAÇÃO DOS BICOS DE DOSAGEM'
WHERE
    ID_OS = 5;
...
/* DEVIDO A UMA FALHA, FORAM GERADOS REGISTROS EM DUPLICIDADE DE UMA DAS OS (ID_OS = 10 E ID_OS = 11) . EXCLUA A DUPLICIDADE GERADA, MANTENDO O PRIMEIRO REGISTRO DA SOLICITAÇÃO */
DELETE FROM OS
WHERE
    ID_OS = 11;

/* O REQUISITANTE 'ENZO' FOI GERADO POR UM ADMINISTRADOR DO BANCO DE DADOS ENQUANTO REALIZAVA TESTES. REMOVA ESTE REGISTRO PARA QUE O BANCO REFLITA APENAS AS OPERAÇÕES DO CLIENTE */
DELETE FROM REQUISITANTE
WHERE
	NOME LIKE 'ENZO';